...
//    import org.buni.meldware.admin.controller.ServiceEvent;
    ...
    public class DeleteMailListMemberCommand extends SequenceCommand implements Command, Responder {

        private var model : ModelLocator = ModelLocator.getInstance();
       
        override public function execute( event : CairngormEvent ) : void {
            trace("in DeleteMailListMemberCommand execute");
            
            var delegate : MailListDelegate = new MailListDelegate( this );   
            delegate.deleteMailListMember( event.data.list, event.data.member );          
        }
       ...
       public function onResult( event : * = null ) : void {            
           if (event.result.status != null) {
              ...
              var e:CairngormEvent = new CairngormEvent(AdminController.MAILLIST_MEMBER_DELETED);
