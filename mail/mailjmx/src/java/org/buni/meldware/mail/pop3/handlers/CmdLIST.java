...
/**
 * LIST generates a list of all messages currently
 * stored in the user's mailbox.
 *
 * @author Eric Daugherty
 * @version $Revision: 1.4 $
 */
public class CmdLIST extends AbstractCommand implements POP3Handler,
        POP3UserMessages {
    ...
    public POP3Response handleRequest(OutputStream out, POP3Request request,
        ...
        log.debug("LIST command handler called");
        ...
        if (verifyState(protocol, writer,
            ...
            if (arguments.length == 0) {
                ...
                int idx = 1;
                for (Long l : sizes) {
                    messageOutput.append(idx + " " + l + "\r\n");
                    totalSize += l.longValue();
                    idx++;
                }

                // Output the response.
                writer.println("+OK " + sizes.size() + " messages ("
                        + totalSize + " octets)");
                writer.print(messageOutput.toString());
                writer.println(".");
            }
            // Output info for one message
            else if (arguments.length == 1) {
                try {
                    int i = Integer.parseInt(arguments[0]);
                    Collection<Long> sizes = folder.getMessageSizes(false, Range.create(i));
                    if (sizes != null && sizes.size() > 0) {
                        writer.println("+OK " + i + " " + sizes.iterator().next());                        
                    } else {
