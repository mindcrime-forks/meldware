...
public class CmdQUIT implements POP3Handler, POP3UserMessages {
    ...
    public POP3Response handleRequest(OutputStream out, POP3Request request,
        ...
        // If the user is in the TRANSACTION state, commit any changes.
        if (protocol.getState() == POP3ProtocolInstance.STATE_TRANSACTION) {
            protocol.setState(POP3ProtocolInstance.STATE_UPDATE);
            try {
                protocol.deleteMarked();
            } catch (MailException me) {
                log.error("Failed commit messages", me);
                writer.println(MESSAGE_COMMIT_FAILED);
                writer.flush();
                return response;
            }
        }
        ...
        // Sign off
        writer.println("+OK " + protocol.getServername()
