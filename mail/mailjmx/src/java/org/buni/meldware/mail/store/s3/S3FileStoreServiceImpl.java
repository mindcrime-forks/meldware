 ...
 * 
 */
public class S3FileStoreServiceImpl extends AbstractStore implements S3FileStoreService
{ 
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.buni.meldware.mail.store.StoreMBean#getInputStream(java.util.long,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.buni.meldware.mail.store.StoreMBean#getOutputStream(java.util.long,
	 *      org.buni.meldware.mail.store.StoreItemMetaData)
	 */
	public OutputStream getOutputStream(Long id, StoreItemMetaData meta) throws StoreException
	{
		try
		{
			return s3FileStore.getOutputStream(id.longValue(), null);
		}
		catch (Exception e)
		{
			throw new StoreException(e);
	 ...
	 * 
	 * @see org.buni.meldware.mail.store.StoreMBean#read(java.util.long, long,
	 *      byte[], int, int)
	 */
    ...
    public int write(Long id, long position, byte[] b, int off, int len) throws StoreException
    {
        ...
        /*
         ...
         if(position > 0)
            throw new StoreException("Amazon S3 does not allow partial writes.  Attempted to beging writing at position '" + position + "'");
         
        if(off > 0)
            throw new StoreException("Amazon S3 does not allow partial writes.  Attempted to beging writing from bytebuffer offset '" + off + "'");
        
        int written = 0;
        try {
            OutputStream oStream = s3FileStore.getOutputStream(id, null);
