...
public class CmdQUIT implements SMTPHandler {
    ...
    public SMTPResponse handleRequest(OutputStream out, SMTPRequest request,
        ...
        SMTPResponse response = new SMTPResponse(request, out, protocol, true);
        PrintWriter writer = response.getWriter();
        writer.println("221 " + protocol.getServername()
                + " closing connection");
        writer.flush();
        request.getInputStream().mark(2);
        //   request.getInputStream().read(new byte[2]);//hack - swallow the last carriage return
        return response;
    }
}
