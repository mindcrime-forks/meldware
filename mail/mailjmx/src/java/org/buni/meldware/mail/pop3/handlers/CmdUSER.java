...
/**
 * USER accepts a username as part
 * of the authentication process.
 *
 * @author Eric Daugherty
 */
public class CmdUSER extends AbstractCommand implements POP3Handler,
        POP3UserMessages {
    ...
    public POP3Response handleRequest(OutputStream out, POP3Request request,
        ...
        boolean tlsRequired = protocol.isRequireTls();
        ...
        if (verifyState(protocol, writer,
            ...
            if (tlsRequired && !protocol.isSecure()) {
                writer.println(MESSAGE_ENCRYPTION_REQUIRED);
                writer.flush();
                return response;
            }

            // Verify a user has not already been specified
            if (protocol.getUsername() != null) {
                writer.println(MESSAGE_USER_ALREADY_SPECIFIED);
                writer.flush();
                return response;
            }

            // Set the user into the protocol if valid.
            ...
            if (username.length() > 0) {
                ...
                writer.println(MESSAGE_USER_INVALID);
            }
        }
