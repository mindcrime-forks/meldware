...
/**
 * UIDL returns the unique id of the
 * requested message, or all messages
 ...
 */
public class CmdUIDL extends AbstractCommand implements POP3Handler,
        POP3UserMessages {

    public final static String COMMAND = "UIDL";
    ...
    public POP3Response handleRequest(OutputStream out, POP3Request request,
        ...
        if (verifyState(protocol, writer,
            ...
            Folder inbox = protocol.getInbox();
            //long[] uids = inbox.getUids();

            // Output all the message ids.
            if (arguments.length == 0) {

                writer.println(MESSAGE_OK);
                int index = 1;
                for (Long uid : inbox) {
                    writer.println(index + " " + uid);                    
                }
                writer.println(".");

            }
            // Output a specific message id.
            else if (arguments.length == 1) {

                int i = Integer.parseInt(arguments[0]);
                long uid = inbox.getUid(i);
                if (uid == -1) {
                //if (i > uids.length) {
                    writer.println(MESSAGE_INVALID_MESSAGE_ID);
                } else {
                    // Write the message id to the client.
                    writer.println(MESSAGE_OK + " " + i + " " + uid);
                }
