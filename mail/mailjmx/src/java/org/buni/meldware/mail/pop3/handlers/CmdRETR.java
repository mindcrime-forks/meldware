...
/**
 * RETR is a request for the server to
 * send a specific message to the client.
 *
 * @author Eric Daugherty
 * @version $Revision: 1.4 $
 */
public class CmdRETR extends AbstractCommand implements POP3Handler,
        POP3UserMessages {

    public final static String COMMAND = "RETR";
    ...
    public POP3Response handleRequest(OutputStream out, POP3Request request,
        ...
        if (verifyState(protocol, out, POP3ProtocolInstance.STATE_TRANSACTION)
                && verifyArgumentCount(arguments, out, 1)) {
            // There should be one argument, which is the message number.
            try {
                ...
                if (message == null) {
                    ...
                    out.write(ENDL);
                    message.write(out, new DotStuffingCopier());
                    out.write(TERM);
                }
